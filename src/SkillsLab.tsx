...
type Lab = "menu" | "errors" | "pairs" | "mediation" | "families" | "dialogue";
const errorBank = [
...
export default function SkillsLab({ level, onComplete }: { level: Cefr; onComplete: (id: string, score: number) => void }) {
  ...
  const reset = (next: Lab) => { setLab(next); setIndex(0); setSelected(null); setCorrect(0); setText(""); setChecked(false); setDialogueStep(0); };
  const finish = (id: string, total: number, value = correct) => { onComplete(`skills:${level}:${id}`, Math.round((value / total) * 100)); reset("menu"); };
  if (lab === "menu") return <section className="skillsHub"><div className="themeHeading"><span><small>LABORATORI PRATICI · {level}</small><h2>Usa l’inglese, non soltanto riconoscerlo</h2></span><b>5 attività</b></div><div className="skillsCards">
