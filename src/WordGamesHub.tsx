...
function MatchingGame({level,saved,onDone,onBack}:{level:Cefr;saved?:Result;onDone:(score:number)=>void;onBack:()=>void}){
 const entries=useMemo(()=>shuffle(wordGameSets[level].crossword).slice(0,8),[level]),[round,setRound]=useState(0),[selected,setSelected]=useState<number|string|null>(null),[correct,setCorrect]=useState(0);
 const entry=entries[round],options=useMemo(()=>{const peers=wordGameSets[level].crossword.filter(item=>item.answer!==entry.answer&&item.kind===entry.kind);return shuffle([entry.answer,...closest(entry.clue,peers,item=>item.clue).map(item=>item.answer)])},[entry,level]),selectedIndex=typeof selected==="number"?selected:-1,isCorrect=selectedIndex>=0&&options[selectedIndex]===entry.answer;
 const choose=(answer:string|number)=>{if(selected!==null)return;const choice=typeof answer==="number"?answer:options.indexOf(answer);setSelected(choice);if(options[choice]===entry.answer)setCorrect(value=>value+1)};
 ...
 return <section className="gameSession"><header><button type="button" onClick={onBack}>← Giochi</button><b>{level}</b></header><span className="eyebrow">ABBINAMENTO · PAROLA E DEFINIZIONE</span><h1>Trova la parola inglese</h1><p className="gameIntro">Leggi la definizione in inglese e scegli la parola corrispondente.</p>{saved&&<small className="gamePrevious">Migliore risultato: {saved.score}% · {saved.attempts} tentativi</small>}<div className="gameRound">Abbinamento {round+1} / {entries.length}</div><div className="matchingClue" lang="en">{entry.clue}</div><div className="matchingOptions">{options.map((option,i)=><button type="button" key={option} disabled={selected!==null} className={answerVisualState(i,selected,options.indexOf(entry.answer))} onClick={()=>choose(option)}>{option}</button>)}</div>{selected!==null&&<div className={`gameFeedback ${isCorrect?"perfect":"incorrect"}`}><strong>{isCorrect?"Abbinamento corretto!":"Risposta non corretta: confronta la soluzione."}</strong><span lang="en">{entry.answer}: {entry.clue}</span></div>}<div className="gameActions">{selected===null?<button type="button" className="gameSkip" onClick={()=>setSelected("")}>Salta questo abbinamento</button>:<button type="button" className="gamePrimary" onClick={next}>{round+1<entries.length?"Prossimo abbinamento":"Concludi la sessione"}</button>}</div></section>
}
...
export function millionaireChoiceState(choice:number,selected:number|null,answer:number){return answerVisualState(choice,selected,answer)}
...
function PrecisionGame({level,saved,onDone,onBack}:{level:Cefr;saved?:Result;onDone:(score:number)=>void;onBack:()=>void}){
 const questions=useMemo(()=>shuffle(semanticPrecision[level]).slice(0,8).map(item=>{const options=shuffle(item.options);return{...item,options,answer:options.indexOf(item.options[item.answer])}}),[level]);
 const[index,setIndex]=useState(0),[selected,setSelected]=useState<number|null>(null),[correct,setCorrect]=useState(0),question=questions[index],isCorrect=selected!==null&&precisionIsCorrect(question,selected);
 const next=()=>{const total=correct+(isCorrect?1:0);if(index+1<questions.length){setCorrect(total);setIndex(value=>value+1);setSelected(null)}else onDone(Math.round(total/questions.length*100))};
 return <section className="gameSession precisionSession"><header><button type="button" onClick={onBack}>← Giochi</button><b>{level}</b></header><span className="eyebrow">LESSICO · PAROLE VICINE</span><h1>Scegli la parola precisa</h1><p className="gameIntro">Tutte le alternative appartengono allo stesso ambito o si assomigliano. Leggi l’intera frase: soltanto una scelta è corretta nella struttura e nel significato.</p>{saved&&<small className="gamePrevious">Migliore risultato: {saved.score}% · {saved.attempts} tentativi</small>}<div className="gameRound">Frase {index+1} / {questions.length}</div><div className="matchingClue" lang="en">{question.prompt}</div><div className="matchingOptions precisionOptions">{question.options.map((option,choice)=><button type="button" lang="en" key={option} disabled={selected!==null} className={selected===null?"":choice===question.answer?"right":choice===selected?"wrong":""} onClick={()=>setSelected(choice)}>{option}</button>)}</div>{selected!==null&&<div className={`gameFeedback ${isCorrect?"perfect":""}`}><strong>{isCorrect?"Scelta precisa.":"Sono parole vicine, ma qui serve un’altra costruzione."}</strong><span>{question.explanationIt}</span><span lang="en">{question.prompt.replace("___",question.options[question.answer])}</span></div>}<div className="gameActions">{selected===null?<button type="button" className="gameSkip" onClick={()=>setSelected(-1)}>Salta questa domanda</button>:<button type="button" className="gamePrimary" onClick={next}>{index+1<questions.length?"Prossima frase":"Vedi il risultato finale"}</button>}</div></section>
}
function NaturalReplyGame({level,saved,onDone,onBack}:{level:Cefr;saved?:Result;onDone:(score:number)=>void;onBack:()=>void}){
 const questions=useMemo(()=>shuffle(naturalReplies[level]).slice(0,8).map(item=>{const options=shuffle(item.options);return{...item,options,answer:options.indexOf(item.options[item.answer])}}),[level]);
 const[index,setIndex]=useState(0),[selected,setSelected]=useState<number|null>(null),[correct,setCorrect]=useState(0),question=questions[index],isCorrect=selected!==null&&naturalReplyIsCorrect(question,selected);
 const next=()=>{const total=correct+(isCorrect?1:0);if(index+1<questions.length){setCorrect(total);setIndex(value=>value+1);setSelected(null)}else onDone(Math.round(total/questions.length*100))};
 return <section className="gameSession naturalReplySession"><header><button type="button" onClick={onBack}>← Giochi</button><b>{level}</b></header><span className="eyebrow">CONVERSAZIONE · RISPOSTA NATURALE</span><h1>Che cosa diresti davvero?</h1><p className="gameIntro">Leggi la situazione e scegli la risposta più naturale, corretta e adatta al tono. Ai livelli avanzati le alternative sono volutamente vicine.</p>{saved&&<small className="gamePrevious">Migliore risultato: {saved.score}% · {saved.attempts} tentativi</small>}<div className="gameRound">Dialogo {index+1} / {questions.length}</div><div className="naturalSituation">{question.situationIt}</div><div className="dialoguePrompt" lang="en"><small>L'ALTRA PERSONA</small><strong>{question.prompt}</strong></div><div className="matchingOptions naturalReplyOptions">{question.options.map((option,choice)=><button type="button" lang="en" key={option} disabled={selected!==null} className={selected===null?"":choice===question.answer?"right":choice===selected?"wrong":""} onClick={()=>setSelected(choice)}>{option}</button>)}</div>{selected!==null&&<div className={`gameFeedback ${isCorrect?"perfect":""}`}><strong>{isCorrect?"Risposta naturale e adatta al contesto.":"La frase scelta non è la più adatta in questa situazione."}</strong><span>{question.explanationIt}</span><span lang="en">Risposta consigliata: {question.options[question.answer]}</span></div>}<div className="gameActions">{selected===null?<button type="button" className="gameSkip" onClick={()=>setSelected(-1)}>Salta questo dialogo</button>:<button type="button" className="gamePrimary" onClick={next}>{index+1<questions.length?"Prossimo dialogo":"Vedi il risultato finale"}</button>}</div></section>
}
export default function WordGamesHub({level,saved,onComplete}:Props){
 ...
 if(game==="precision")return <PrecisionGame level={level} saved={saved[id(game)]} onDone={score=>onComplete(id(game),score)} onBack={()=>setGame(null)}/>;
 if(game==="natural")return <NaturalReplyGame level={level} saved={saved[id(game)]} onDone={score=>onComplete(id(game),score)} onBack={()=>setGame(null)}/>;
 ...
 return <section className="wordGamesHub">
  <div className="themeHeading"><span><small>GIOCHI IN INGLESE · {level}</small><h2>Impara giocando</h2></span><b>11 sessioni</b></div>
  <p className="readingHubIntro">Undici giochi autonomi con vocabolario, conversazione e strutture calibrati per il livello {level}. Le istruzioni sono in italiano; indizi e contenuti da risolvere restano in inglese.</p>
  <div className="gameCards">
   ...
   <button type="button" onClick={()=>setGame("precision")}><b>≋</b><strong>La parola precisa</strong><p>Distingui parole vicine usando significato, costruzione e contesto.</p><small>10–15 min · 8 su {semanticPrecision[level].length} frasi{saved[id("precision")]?` · record ${saved[id("precision")].score}%`:""}</small></button>
   <button type="button" onClick={()=>setGame("natural")}><b>“ ”</b><strong>La risposta naturale</strong><p>Scegli cosa dire davvero in dialoghi quotidiani, professionali e formali.</p><small>10–15 min · 8 su {naturalReplies[level].length} dialoghi{saved[id("natural")]?` · record ${saved[id("natural")].score}%`:""}</small></button>
