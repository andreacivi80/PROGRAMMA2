...
function Hangman({level,saved,onDone,onBack}:{level:Cefr;saved?:Result;onDone:(score:number)=>void;onBack:()=>void}){
 ...
 return <section className="gameSession">
  ...
  <p className="gameIntro">Usa l’indizio in inglese. Completa cinque frasi adatte al livello; puoi saltarne una senza uscire dalla sessione.</p>
  {saved&&<small className="gamePrevious">Migliore risultato: {saved.score}% · {saved.attempts} tentativi</small>}
...
export default function WordGamesHub({level,saved,onComplete}:Props){
 ...
 return <section className="wordGamesHub">
  <div className="themeHeading"><span><small>GIOCHI IN INGLESE · {level}</small><h2>Impara giocando</h2></span><b>6 sessioni</b></div>
  <p className="readingHubIntro">Sei giochi autonomi con vocabolario e strutture calibrati per il livello {level}. Le istruzioni sono in italiano; indizi e contenuti da risolvere restano in inglese.</p>
  <div className="gameCards">
   ...
   <button type="button" onClick={()=>setGame("hangman")}><b>_?</b><strong>Frasi dell’impiccato</strong><p>Scegli le lettere e completa cinque frasi adatte al tuo livello.</p><small>8–12 min · 5 frasi{saved[id("hangman")]?` · record ${saved[id("hangman")].score}%`:""}</small></button>
   <button type="button" onClick={()=>setGame("wordorder")}><b>1·2</b><strong>Ordine delle parole</strong><p>Ricostruisci frasi inglesi complete toccando le parole nell’ordine corretto.</p><small>8–12 min · 5 frasi{saved[id("wordorder")]?` · record ${saved[id("wordorder")].score}%`:""}</small></button>
