...
export default function WordGamesHub({level,saved,onComplete}:Props){
 ...
 return <section className="wordGamesHub">
  ...
  <div className="gameCards">
   <button type="button" onClick={()=>setGame("crossword")}><b>▦</b><strong>Mini Crossword</strong><p>Leggi le definizioni inglesi e collega le parole nella griglia.</p><small>10–15 min · {wordGameSets[level].crossword.length} definizioni{saved[id("crossword")]?` · record ${saved[id("crossword")].score}%`:""}</small></button>
   <button type="button" onClick={()=>setGame("hangman")}><b>_?</b><strong>Hangman Phrases</strong><p>Scegli le lettere e completa cinque frasi adatte al tuo livello.</p><small>8–12 min · 5 frasi{saved[id("hangman")]?` · record ${saved[id("hangman")].score}%`:""}</small></button>
   <button type="button" onClick={()=>setGame("wordorder")}><b>1·2</b><strong>Word Order</strong><p>Ricostruisci frasi inglesi complete toccando le parole nell’ordine corretto.</p><small>8–12 min · 5 frasi{saved[id("wordorder")]?` · record ${saved[id("wordorder")].score}%`:""}</small></button>
   <button type="button" onClick={()=>setGame("matching")}><b>↔</b><strong>Match the Meaning</strong><p>Abbina ogni definizione inglese alla parola corretta.</p><small>8–12 min · 8 abbinamenti{saved[id("matching")]?` · record ${saved[id("matching")].score}%`:""}</small></button>
   <button type="button" onClick={()=>setGame("millionaire")}><b>€?</b><strong>English Millionaire</strong><p>Scala i livelli con dieci domande e l’aiuto 50:50.</p><small>12–18 min · 10 domande{saved[id("millionaire")]?` · record ${saved[id("millionaire")].score}%`:""}</small></button>
   <button type="button" onClick={()=>setGame("trivia")}><b>●</b><strong>Trivia Quest</strong><p>Completa quattro categorie: parole, definizioni, frasi e inglese quotidiano.</p><small>12–18 min · 12 domande{saved[id("trivia")]?` · record ${saved[id("trivia")].score}%`:""}</small></button>
  </div>
 </section>
}
