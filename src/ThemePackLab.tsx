...
function speak(text:string,onWord?:(index:number)=>void){
 ...
 utterance.onboundary=event=>{if(event.name==="word")onWord?.(text.slice(0,event.charIndex).trim().split(/\s+/).filter(Boolean).length)};
 utterance.onend=utterance.onerror=()=>onWord?.(-1);
...
export default function ThemePackLab({pack,badge,previous,onClose,onComplete}:Props){
 ...
 const startQuiz=()=>{speechSynthesis?.cancel();setScenarioWord(-1);setQuiz(buildQuiz(pack));setIndex(0);setSelected(null);setCorrect(0);reported.current=false;setPhase("quiz");scrollTo(0,0)};
