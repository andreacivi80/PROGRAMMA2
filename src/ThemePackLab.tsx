...
import {applyDialogueVoice,dialogueRole,dialogueVoicePair} from "./speechVoices";
...
function speak(text:string,onWord?:(index:number)=>void){
 ...
 const turns=parseDialogue(text),pair=dialogueVoicePair(speechSynthesis.getVoices(),text);
 ...
 const playTurn=()=>{if(scenarioPlayback!==playback)return;const turn=turns[turnIndex],utterance=new SpeechSynthesisUtterance(turn.text);applyDialogueVoice(utterance,pair,dialogueRole(turn.speaker,turnIndex));utterance.rate=.92;let localIndex=0;
...
export default function ThemePackLab({pack,badge,previous,onClose,onComplete}:Props){
 ...
 const levelTone=useMemo(()=>`level-${pack.level.toLowerCase()}`,[pack.level]),scenarioTurns=useMemo(()=>parseDialogue(pack.scenario.text),[pack.scenario.text]); useEffect(()=>{const box=scenarioRef.current;if(!box)return;if(scenarioWord<0){box.scrollTo({top:0,behavior:"auto"});return}const active=box.querySelector<HTMLElement>(".active");if(!active)return;const boxRect=box.getBoundingClientRect(),activeRect=active.getBoundingClientRect(),insideTop=activeRect.top-boxRect.top+box.scrollTop,target=Math.max(0,Math.min(box.scrollHeight-box.clientHeight,insideTop-(box.clientHeight-activeRect.height)/2));box.scrollTo({top:target,behavior:"smooth"})},[scenarioWord]);
