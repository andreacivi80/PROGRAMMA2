...
export default function LearningCoach({ level, goal, skills, clusters, phrases, monthly, weeklyDone, prerequisite, onGoal, onMicro, onNew, onPrerequisite, onReview, onReading, onSimulation, onRemovePhrase, onMonthly, onWeekly }: {
  ...
  const [weeklyReview, setWeeklyReview] = useState<{ corrected: string; notes: string[] } | null>(null);
  ...
  return (
    <section className="learningCoach">
      ...
      {tab === "plan" && <>
        ...
        <div className="weeklyChallenge"><span><small>SFIDA DELLA SETTIMANA</small><h3>{weeklyDone ? "Sfida completata" : goal.includes("lavoro") ? "Scrivi un aggiornamento professionale" : goal.includes("Viaggi") ? "Risolvi un imprevisto durante un viaggio" : "Racconta una decisione e spiegane il motivo"}</h3></span>{weeklyDone ? <p>Hai già completato la missione. La prossima settimana ne arriverà una nuova.</p> : <><p>Scrivi almeno 35 parole in inglese. Usa un collegamento come <b lang="en">because</b>, <b lang="en">however</b> o <b lang="en">although</b>.</p><textarea value={weeklyText} onChange={event => { setWeeklyText(event.target.value); setWeeklyReview(null); }} placeholder="Scrivi qui la tua risposta…" /><button disabled={weeklyText.trim().split(/\s+/).length < 35} onClick={() => setWeeklyReview(analyzeLocalWriting(weeklyText))}>Controlla prima di completare</button>{weeklyReview && <section className="weeklyCorrection"><strong>{weeklyReview.corrected === weeklyText.trim() ? "Il controllo locale non rileva errori frequenti." : "Rivedi questi punti prima di concludere."}</strong>{weeklyReview.notes.map(note => <span key={note}>{note}</span>)}{weeklyReview.corrected !== weeklyText.trim() && <p lang="en">{weeklyReview.corrected}</p>}<button onClick={() => onWeekly(weeklyReview.corrected)}>Conferma la sfida</button></section>}</>}</div>
      </>}
