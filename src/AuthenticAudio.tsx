...
import { getAudioRate, saveAudioRate, type AudioRate } from "./preferences";

export default function AuthenticAudio({
  url,
  label,
  duration,
}: {
  ...
  return (
    <section className="authenticAudio">
      <audio ref={audio} src={url} preload="metadata" onEnded={stop} />
      <div>
        <small>CAMPIONE AUTENTICO · VOCE REALE · {duration}</small>
        <strong>{label}</strong>
      </div>
      <div className="authenticAudioActions">
      ...
      <div className="authenticSpeed" aria-label="Velocità campione">
