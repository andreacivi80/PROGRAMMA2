(()=>{
  ...
  const state={requests:0,retries:0,recovered:0,failures:0,timeouts:0,deduplicated:0,discarded:0,lastFailure:"",lastSuccessAt:"",lastLatencyMs:0,lastRequestId:""};
  const inFlight=new Map(),latestSequence=new Map();
  ...
  const fetchJson=(url,options={},settings={})=>{
    const method=String(options.method||"GET").toUpperCase(),dedupe=settings.dedupe!==false&&method==="GET",key=dedupe?`${method}:${url}`:"";
    ...
    const promise=runFetch(url,options,settings,key,sequence).finally(()=>{if(key&&inFlight.get(key)===promise)inFlight.delete(key)});
    if(key)inFlight.set(key,promise);return promise;
  };
  const diagnostics=()=>Object.freeze({...state,inFlight:inFlight.size});
  window.TechnicsDataClient=Object.freeze({parseText,read,fetchJson,diagnostics,incompleteMessage,version:"1.5.0"});
  document.documentElement.dataset.dataClient="1.5.0";
})();
