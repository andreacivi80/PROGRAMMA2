...
(() => {
  ...
  function verifiedCommitment(resolved,expectedCode){
    ...
    if(String(resolved?.code||'').trim().toUpperCase()!==String(expectedCode||'').trim().toUpperCase()||!rows.length||!Number.isFinite(total)||!Number.isFinite(committed)||Math.abs(total-committed)>0.01||rows.some(row=>!String(row.lot||'').trim()||!Number.isFinite(Number(row.quantity))||(unit&&row.unit&&String(row.unit).trim().toUpperCase()!==unit)))return null;
    const rowKeys=rows.map(row=>[row.op,row.ov,row.lot,row.deliveryDate,row.quantity,String(row.unit||unit).trim().toUpperCase()].map(value=>String(value??'').trim()).join('|'));
  ...
  function openCommitments(item,summary){
    ...
    tbody.innerHTML='<tr><td colspan="5">Caricamento dati aggiornati…</td></tr>';
    flag.checked=false;flag.disabled=true;flag.onchange=null;
    ...
    const paint=resolved=>{if(requestSequence!==commitmentRequestSequence)return;const verified=verifiedCommitment(resolved,item.code);if(!verified)throw Error('Dati aggiornati non coerenti.');currentItem=resolved;const {detail,rows,total}=verified,overdueRows=rows.filter(row=>deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0);overlay.querySelector('h3').textContent=`${resolved.code||''} · ${resolved.name||resolved.description||''}`;const render=()=>{const visible=flag.checked?overdueRows:rows;tbody.innerHTML=visible.map(row=>`<tr${deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0?' class="inventorycommitmentoverdue" data-delivery-overdue="true"':''}><td>${esc(row.op||'—')}</td><td>${esc(row.ov||'—')}</td><td>${esc(row.lot||'—')}</td><td>${esc(date(row.deliveryDate))}</td><td>${esc(format(Number(row.quantity),row.unit||summary.unit))}</td></tr>`).join('')};flag.disabled=false;flag.onchange=render;render();const overdueTotal=overdueRows.reduce((sum,row)=>sum+Number(row.quantity),0);overlay.querySelector('[data-inventory-commitments-overdue-total]').textContent=format(overdueTotal,detail.unit||summary.unit);overlay.querySelector('[data-inventory-commitments-total]').textContent=format(total,detail.unit||summary.unit);overlay.querySelector('[data-inventory-commitments-total-label]').innerHTML='Totale impegnato <span class="inventorycommitmentverified">✓ Somma verificata</span>';check.className='inventorycommitmentok';check.textContent='Dati aggiornati e somma verificata.';if(preview){preview.disabled=false;preview.onclick=()=>openCommittedPrint(currentItem)}};
    loadNativeCommitmentItem(item).then(paint).catch(error=>{if(requestSequence!==commitmentRequestSequence)return;tbody.innerHTML='<tr><td colspan="5">Dati aggiornati non disponibili.</td></tr>';flag.disabled=true;check.className='inventorycommitmenterror';check.textContent=error?.message||'Dati aggiornati non disponibili.';if(preview){preview.disabled=true;preview.onclick=null}});
  }
