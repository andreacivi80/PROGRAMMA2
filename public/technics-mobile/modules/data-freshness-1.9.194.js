(()=>{
  "use strict";
  const state={dataTime:"",serverTime:"",source:"",latencyMs:0,cached:false,lastReceivedAt:"",receivedMonotonic:0,healthVerifiedMonotonic:0,level:"waiting",nodeId:"",nodeRole:""};
  ...
  const paint=()=>{
    ...
    const stamp=parse(state.dataTime)||parse(state.serverTime),serverStamp=parse(state.serverTime),elapsed=state.receivedMonotonic?Math.max(0,performance.now()-state.receivedMonotonic):0,age=stamp&&serverStamp?Math.max(0,Math.round((serverStamp-stamp+elapsed)/1000)):stamp&&state.receivedMonotonic?Math.max(0,Math.round(elapsed/1000)):Infinity;
    const verified=Number(state.healthVerifiedMonotonic||0)>0&&performance.now()-Number(state.healthVerifiedMonotonic)<60000;
  ...
  document.addEventListener("technics:data-success",event=>{Object.assign(state,event.detail||{}, {lastReceivedAt:new Date().toISOString(),receivedMonotonic:performance.now()});paint()});
  document.addEventListener("technics:health-ready",event=>{state.healthVerifiedMonotonic=performance.now();if(event.detail?.nodeId)state.nodeId=String(event.detail.nodeId);paint()});
  document.addEventListener("technics:health",event=>{if(event.detail?.ok){state.healthVerifiedMonotonic=performance.now();if(event.detail?.nodeId)state.nodeId=String(event.detail.nodeId);paint()}});
  const start=()=>{ensure();paint();clearInterval(timer);timer=setInterval(paint,1000)};
  document.readyState==="loading"?document.addEventListener("DOMContentLoaded",start,{once:true}):start();
  window.TechnicsDataFreshness=Object.freeze({diagnostics:()=>Object.freeze({...state}),version:"1.9.194"});
  document.documentElement.dataset.dataFreshness="1.9.194";
})();
