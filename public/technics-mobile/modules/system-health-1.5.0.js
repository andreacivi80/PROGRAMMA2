(()=>{
  ...
  const state={ok:false,database:null,failures:0,lastCheck:"",latencyMs:0,version:"",message:"Avvio controllo"};
  let cycle=0;
  const paint=()=>{
    const node=document.getElementById("net");if(!node)return;
    const healthy=state.ok&&state.database!==false;
  ...
  const check=async()=>{
    cycle++;const deep=cycle%3===0,started=performance.now(),controller=new AbortController(),timer=setTimeout(()=>controller.abort(),6500);
    try{
      ...
      state.ok=true;state.failures=0;state.version=String(payload.version);state.database=deep?Boolean(payload.database?.ok):state.database;state.message="Sistema operativo";
  ...
  window.TechnicsSystemHealth=Object.freeze({check,diagnostics:()=>Object.freeze({...state}),version:"1.5.0"});
  document.documentElement.dataset.systemHealth="1.5.0";
