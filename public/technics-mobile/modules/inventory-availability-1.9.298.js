...
(() => {
  ...
  function openCommitments(item,summary){
    ...
    paint(item);overlay.classList.remove('hidden');overlay.querySelector('[data-inventory-commitments-close]').focus({preventScroll:true});const rows=item.commitmentDetail?.rows||[],needsNative=rows.some(row=>!row.lot);if(needsNative)loadNativeCommitmentItem(item).then(paint).catch(()=>{});
  }
