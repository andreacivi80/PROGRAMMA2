(()=>{
  ...
  const verifiedEmergencyResponse=(payload,response)=>{const cache=String(response?.headers?.get?.("X-Technics-Cache")||""),degraded=String(response?.headers?.get?.("X-Technics-Degraded-Read-Only")||""),lastSyncAt=String(response?.headers?.get?.("X-Technics-Last-Sync-At")||""),gateway=String(response?.headers?.get?.("X-Technics-Gateway")||""),at=Date.parse(lastSyncAt),serverAt=Date.parse(payload?.meta?.serverTime||response?.headers?.get?.("X-Technics-Server-Time")||""),age=serverAt-at;return cache==="emergency-stale-readonly"&&degraded==="true"&&gateway==="stable-worker-emergency-cache"&&payload?.degradedReadOnly===true&&payload?.stale===true&&payload?.offline===true&&payload?.cacheSource==="gateway-last-known-good"&&payload?.lastSyncAt===lastSyncAt&&Number.isFinite(at)&&Number.isFinite(serverAt)&&age>=0&&age<=5*60*1000};
  const verifiedGatewayError=(payload,response)=>payload?.ok===false&&payload?.gateway===true&&response?.ok===false&&String(response?.headers?.get?.("X-Technics-Gateway")||"")==="stable-worker";
  const validateMeta=(payload,expectedId,response)=>{
    if(!payload.meta){if(verifiedGatewayError(payload,response)){Object.defineProperty(payload,"__technicsGatewayError",{value:true,enumerable:false});return payload}state.integrityFailures++;const error=new Error("Risposta priva della tracciabilità Technics.");error.incompleteResponse=true;throw error}
    ...
    if(!Number.isFinite(serverTime))throw new Error("Risposta dati con data non valida.");
  ...
  window.TechnicsDataClient=Object.freeze({parseText,read,fetchJson,invalidate,diagnostics,incompleteMessage,version:"1.9.290"});
  document.documentElement.dataset.dataClient="1.9.290";
})();
