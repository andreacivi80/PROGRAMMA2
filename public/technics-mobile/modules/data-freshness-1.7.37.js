(()=>{
  ...
  document.addEventListener("technics:health",event=>{if(event.detail?.ok){state.healthVerifiedAt=Date.now();if(event.detail?.nodeId)state.nodeId=String(event.detail.nodeId);paint()}});
  const start=()=>{ensure();paint();clearInterval(timer);timer=setInterval(paint,1000)};
  document.readyState==="loading"?document.addEventListener("DOMContentLoaded",start,{once:true}):start();
  window.TechnicsDataFreshness=Object.freeze({diagnostics:()=>Object.freeze({...state}),version:"1.9.32"});
  document.documentElement.dataset.dataFreshness="1.9.32";
})();
