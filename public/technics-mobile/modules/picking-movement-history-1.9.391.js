(() => {
  ...
  style.textContent += '.pickmovementinfo{font-style:italic!important}';
  ...
  const compactOperator = movement => {
    const legacyName=String(movement.operator||movement.operatorSurname||'').trim().replace(/\s+/g,' ').toLocaleUpperCase('it-IT');
    if(legacyName==='MIGLIORATI STEFANO')return 'MIGLIORATI S.';
    // Exact aliases approved against the read-only operator audit; never infer name order.
    const aliases = {'BONANOMI EMANUELE':'BONANOMI E.','CRIPPA ROSSELLA':'CRIPPA R.','PIROTTA FEDERICA':'PIROTTA F.','SCAPOLAN DENIS':'SCAPOLAN D.','GOLDONETTO VITTORIO':'GOLDONETTO V.','FRISOLI SIMONE':'FRISOLI S.','CREPPI FEDERICO':'CREPPI F.'};
    if (Object.hasOwn(aliases, legacyName)) return aliases[legacyName];
    const surname = String(movement.operatorSurname || '').trim();
    const given = String(movement.operatorGivenName || '').trim();
    if (surname && given) return surname.toLocaleUpperCase('it-IT') + ' ' + Array.from(given)[0].toLocaleUpperCase('it-IT') + '.';
    const parts = legacyName.split(' ');
    if (parts.length >= 2 && parts.every(part => /^\p{L}+$/u.test(part))) return parts.slice(0, -1).join(' ') + ' ' + Array.from(parts.at(-1))[0] + '.';
