...
(() => {
  ...
  function committedPreviewUrls(item){const detail=item?.commitmentDetail||{},source=Array.isArray(detail.rows)?detail.rows:[];if(!source.length)throw Error('Nessun materiale impegnato da visualizzare.');const perPage=18,pageCount=Math.ceil(source.length/perPage),urls=[];for(let page=0;page<pageCount;page++){const rows=source.slice(page*perPage,(page+1)*perPage),W=2480,H=3508,left=130,width=2220,columns=[250,220,320,430,500,500];let y=430;const text=(value,x,top,w,size=25,weight=400,fill='#163e35')=>`<text x="${x+w/2}" y="${top}" font-family="Arial,sans-serif" font-size="${size}" font-weight="${weight}" fill="${fill}" text-anchor="middle">${committedPreviewXml(value)}</text>`,out=[`<?xml version="1.0" encoding="UTF-8"?>`,`<svg xmlns="http://www.w3.org/2000/svg" width="210mm" height="297mm" viewBox="0 0 ${W} ${H}"><rect width="${W}" height="${H}" fill="#fff"/>`,text('I.R.A. ISTITUTO RICERCHE APPLICATE S.p.A.',left,130,width,42,700),text('MATERIALE IMPEGNATO',left,235,width,62,700),`<rect x="${left}" y="285" width="${width}" height="100" rx="8" fill="#e7f1ec"/>`,text(`${item.code||''} · ${item.name||item.description||''}`,left+10,350,width-20,31,700)];let x=left;['OP','OV','LOTTO','LAYOUT','DATA DI CONSEGNA','IMPEGNATO'].forEach((label,index)=>{out.push(`<rect x="${x}" y="${y}" width="${columns[index]}" height="75" fill="#174f45"/>`,text(label,x,y+48,columns[index],18,700,'#fff'));x+=columns[index]});y+=75;rows.forEach((row,index)=>{const overdue=deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0,values=[row.op||'—',row.ov||'—',row.lot||'—',commitmentLayouts(row),date(row.deliveryDate),format(Number(row.quantity),row.unit||detail.unit||item.unit)];x=left;out.push(`<rect x="${left}" y="${y}" width="${width}" height="105" fill="${index%2?'#f7faf8':'#fff'}" stroke="#c7d7cf" stroke-width="2"/>`);values.forEach((value,column)=>{out.push(text(value,x+3,y+65,columns[column]-6,column===3?18:22,column<4?700:400,overdue?'#b42318':'#516e65'));x+=columns[column];if(column<5)out.push(`<line x1="${x}" y1="${y}" x2="${x}" y2="${y+105}" stroke="#c7d7cf" stroke-width="2"/>`)});y+=105});if(page===pageCount-1){const totalY=Math.min(y+30,3200);out.push(`<rect x="${left}" y="${totalY}" width="${width}" height="85" fill="#e7f1ec"/>`,text(`TOTALE IMPEGNATO  ${format(Number(detail.totalQuantity),detail.unit||item.unit)}`,left,totalY+54,width,28,700))}out.push(text(`Pagina ${page+1} di ${pageCount}`,left,3400,width,20,400,'#516e65'),'</svg>');urls.push(URL.createObjectURL(new Blob([out.join('')],{type:'image/svg+xml;charset=utf-8'})))}return urls}
  ...
  async function loadNativeCommitmentItem(item){const code=String(item?.code||'').trim();if(!code)return item;const {response,payload}=await TechnicsDataClient.fetchJson(globalThis.__technicsBridgeUrl+'/api/items/lookup?code='+encodeURIComponent(code)+'&commitmentDetails=native&fresh='+Date.now(),{cache:'no-store',headers:{'Cache-Control':'no-store'}},{attempts:1,cacheMs:0,message:'Dettaglio Lotto/Layout temporaneamente non disponibile.'});const resolved=payload?.item;if(!response.ok||payload?.ok!==true||!resolved||String(resolved.code||'').trim().toUpperCase()!==code.toUpperCase()||!Array.isArray(resolved.commitmentDetail?.rows)||!resolved.commitmentDetail.rows.length)throw Error(payload?.error||'Dettaglio Lotto/Layout non disponibile.');return resolved}
  function openCommitments(item,summary){
    const overlay=commitmentDialog();let currentItem=item;
    const paint=resolved=>{currentItem=resolved;const detail=resolved.commitmentDetail||{},rows=Array.isArray(detail.rows)?detail.rows:[];overlay.querySelector('h3').textContent=`${resolved.code||''} · ${resolved.name||resolved.description||''}`;const overdueRows=rows.filter(row=>deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0),flag=overlay.querySelector('[data-inventory-overdue-only]'),tbody=overlay.querySelector('tbody');const render=()=>{const visible=flag.checked?overdueRows:rows;tbody.innerHTML=visible.map(row=>`<tr${deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0?' class="inventorycommitmentoverdue" data-delivery-overdue="true"':''}><td>${esc(row.op||'—')}</td><td>${esc(row.ov||'—')}</td><td>${esc(row.lot||'—')}</td><td>${esc(commitmentLayouts(row))}</td><td>${esc(date(row.deliveryDate))}</td><td>${esc(format(Number(row.quantity),row.unit||summary.unit))}</td></tr>`).join('')};flag.checked=false;flag.onchange=render;render();const overdueTotal=overdueRows.reduce((sum,row)=>sum+Number(row.quantity||0),0);overlay.querySelector('[data-inventory-commitments-overdue-total]').textContent=format(overdueTotal,detail.unit||summary.unit);overlay.querySelector('[data-inventory-commitments-total]').textContent=format(Number(detail.totalQuantity),detail.unit||summary.unit);const totalLabel=overlay.querySelector('[data-inventory-commitments-total-label]');totalLabel.innerHTML=detail.matchesInventory?'Totale impegnato <span class="inventorycommitmentverified">✓ Somma verificata</span>':'Totale impegnato <span class="inventorycommitmentunverified">⚠ Da verificare</span>';const preview=overlay.querySelector('[data-inventory-commitments-preview]');if(preview)preview.onclick=()=>openCommittedPrint(currentItem)};
    paint(item);overlay.classList.remove('hidden');overlay.querySelector('[data-inventory-commitments-close]').focus({preventScroll:true});const rows=item.commitmentDetail?.rows||[],needsNative=rows.some(row=>!row.lot||!commitmentLayouts(row)||commitmentLayouts(row)==='—');if(needsNative)loadNativeCommitmentItem(item).then(paint).catch(()=>{});
  }
