(() => {
  ...
  overlay.hidden = true;
  overlay.innerHTML = '<section class="pickmovementpanel" role="dialog" aria-modal="true" aria-labelledby="pickmovementtitle"><div class="pickmovementhead"><h2 id="pickmovementtitle">Elenco movimenti di magazzino</h2><button type="button" aria-label="Chiudi">×</button></div><p class="pickmovementidentity"></p><p class="pickmovementhint" hidden style="font-size:10px;margin:4px 0">Scorri orizzontalmente per leggere tutte le colonne ↔</p><div class="pickmovementscroll"></div></section>';
  ...
  const show = async (code, lot) => {
    ...
    try {
      ...
      overlay.querySelector('.pickmovementhint').hidden = table.scrollWidth <= area.clientWidth;
      if (rows.length) {
