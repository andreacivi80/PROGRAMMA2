(() => {
  ...
  const renderDocumentChecklist = (section, payload, articleId) => {
    ...
    section.innerHTML = `<h4>DOCUMENTI · CHECKLIST E REVISIONI</h4><div class="formuladoccheck"><div class="formuladocprogress"><span>Analisi contenuto reale</span><b>${payload?.complete ? "COMPLETA" : `${analyzed}/${documents.length} · IN CORSO`}</b></div><div class="formuladocbadges">${checklistLabels.map((label) => { const latest = latestByCategory.get(label); return latest ? `<button type="button" class="formuladocbadge present" ${documentActionAttributes(latest, articleId)} title="Apri l'ultima versione: ${esc(latest.name)}">✓ ${esc(label)}</button>` : `<span class="formuladocbadge">${esc(label)}</span>`; }).join("")}</div><input class="formuladocsearch" type="search" placeholder="Cerca documento" aria-label="Cerca documento, categoria o revisione"></div><div class="formuladoclist">${current.length ? current.map((item) => documentButton(item, articleId)).join("") : '<div class="formulaempty">Nessun file apribile collegato.</div>'}</div>${previous.length ? `<details class="formuladocprevious"><summary>REVISIONI PRECEDENTI · ${previous.length}</summary><div class="formuladoclist">${previous.map((item) => documentButton(item, articleId, true)).join("")}</div></details>` : ""}`;
  };
