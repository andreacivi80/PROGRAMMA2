...
(() => {
  ...
  const needleCommittedItem=item=>/\b(?:AGO|AGHI|TSK)\b/i.test(`${item?.name||''} ${item?.description||''}`);
  const commitmentRows=detail=>Array.isArray(detail?.rows)?detail.rows.map(row=>({op:row.op,ov:row.ov,lot:row.lot,expiry:row.expiry,location:String(row.location||(Array.isArray(row.locations)?row.locations.find(value=>String(value??'').trim()):'')||'').trim(),productionOrderId:row.productionOrderId,productionOrderYear:row.productionOrderYear,identitySignature:row.identitySignature,deliveryDate:row.deliveryDate,quantity:row.quantity,unit:row.unit})):[];
  function commitmentDialog(){
    let overlay=document.getElementById('inventoryCommitmentOverlay');if(overlay)return overlay;
    overlay=document.createElement('div');overlay.id='inventoryCommitmentOverlay';overlay.className='inventorycommitmentoverlay hidden';overlay.innerHTML='<section class="inventorycommitmentdialog" role="dialog" aria-modal="true" aria-labelledby="inventoryCommitmentTitle"><header><div><small>DETTAGLIO IMPEGNATO</small><h3 id="inventoryCommitmentTitle"></h3></div><button type="button" data-inventory-commitments-close aria-label="Chiudi">×</button></header><label class="inventorycommitmentfilter"><input type="checkbox" data-inventory-overdue-only> Data consegna trascorsa</label><div class="inventorycommitmentscroll"><table><thead><tr><th>OP</th><th>OV</th><th>Lotto / Layout</th><th>Data di consegna</th><th>Impegnato</th></tr></thead><tbody></tbody><tfoot><tr class="inventorycommitmentoverduetotal"><th colspan="4">Totale con data di consegna trascorsa</th><th data-inventory-commitments-overdue-total></th></tr><tr><th colspan="4" data-inventory-commitments-total-label>Totale impegnato</th><th data-inventory-commitments-total></th></tr></tfoot></table></div><p data-inventory-commitments-check></p><footer><button type="button" class="inventorycommitmentpreview" data-inventory-commitments-preview>Anteprima / stampa</button></footer></section>';
  ...
  function configureCommitmentColumns(overlay,showLotExpiry){
    const count=showLotExpiry?6:5,summarySpan=count-1;overlay.dataset.inventoryLotExpiry=showLotExpiry?'true':'false';
    overlay.querySelector('thead tr').innerHTML=showLotExpiry?'<th>OP</th><th>OV</th><th>Lotto / Ub.</th><th>Consegna</th><th>Scadenza</th><th>Impegnato</th>':'<th>OP</th><th>OV</th><th>Lotto / Layout</th><th>Data di consegna</th><th>Impegnato</th>';
    overlay.querySelectorAll('tfoot tr th:first-child').forEach(cell=>cell.colSpan=summarySpan);return count;
  }
  ...
  function committedPreviewUrls(item){const detail=item?.commitmentDetail||{},source=commitmentRows(detail);if(!source.length)throw Error('Nessun materiale impegnato da visualizzare.');const showLotExpiry=needleCommittedItem(item),perPage=18,pageCount=Math.ceil(source.length/perPage),urls=[];for(let page=0;page<pageCount;page++){const rows=source.slice(page*perPage,(page+1)*perPage),W=2480,H=3508,left=130,width=2220,columns=showLotExpiry?[230,190,330,400,490,580]:[270,240,420,620,670],labels=showLotExpiry?['OP','OV','LOTTO','SCADENZA LOTTO','DATA DI CONSEGNA','IMPEGNATO']:['OP','OV','LOTTO','DATA DI CONSEGNA','IMPEGNATO'];let y=430;const text=(value,x,top,w,size=25,weight=400,fill='#163e35')=>`<text x="${x+w/2}" y="${top}" font-family="Arial,sans-serif" font-size="${size}" font-weight="${weight}" fill="${fill}" text-anchor="middle">${committedPreviewXml(value)}</text>`,out=[`<?xml version="1.0" encoding="UTF-8"?>`,`<svg xmlns="http://www.w3.org/2000/svg" width="210mm" height="297mm" viewBox="0 0 ${W} ${H}"><rect width="${W}" height="${H}" fill="#fff"/>`,text('I.R.A. ISTITUTO RICERCHE APPLICATE S.p.A.',left,130,width,42,700),text('MATERIALE IMPEGNATO',left,235,width,62,700),`<rect x="${left}" y="285" width="${width}" height="100" rx="8" fill="#e7f1ec"/>`,text(`${item.code||''} · ${item.name||item.description||''}`,left+10,350,width-20,31,700)];const raw=Number(detail.rawInventoryCommittedQuantity);if(detail.rawInventoryCommittedQuantity!=null&&Number.isFinite(raw)&&Math.abs(raw-Number(detail.totalQuantity))>0.01)out.push(text(`ATTENZIONE: INVENTARIO ${format(raw,detail.unit||item.unit)} · IMPEGNI QUALIFICATI ${format(Number(detail.totalQuantity),detail.unit||item.unit)}`,left,412,width,20,700,'#b42318'));let x=left;labels.forEach((label,index)=>{out.push(`<rect x="${x}" y="${y}" width="${columns[index]}" height="75" fill="#174f45"/>`,text(label,x,y+48,columns[index],showLotExpiry?16:18,700,'#fff'));x+=columns[index]});y+=75;rows.forEach((row,index)=>{const overdue=deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0,values=showLotExpiry?[row.op||'—',row.ov||'—',row.lot||'—',date(row.expiry),date(row.deliveryDate),format(Number(row.quantity),row.unit||detail.unit||item.unit)]:[row.op||'—',row.ov||'—',row.lot||'—',date(row.deliveryDate),format(Number(row.quantity),row.unit||detail.unit||item.unit)];x=left;out.push(`<rect x="${left}" y="${y}" width="${width}" height="105" fill="${index%2?'#f7faf8':'#fff'}" stroke="#c7d7cf" stroke-width="2"/>`);values.forEach((value,column)=>{out.push(text(value,x+3,y+65,columns[column]-6,showLotExpiry?20:22,column<3?700:400,overdue?'#b42318':'#516e65'));x+=columns[column];if(column<values.length-1)out.push(`<line x1="${x}" y1="${y}" x2="${x}" y2="${y+105}" stroke="#c7d7cf" stroke-width="2"/>`)});y+=105});if(page===pageCount-1){const totalY=Math.min(y+30,3200);out.push(`<rect x="${left}" y="${totalY}" width="${width}" height="85" fill="#e7f1ec"/>`,text(`TOTALE DETTAGLIATO  ${format(Number(detail.totalQuantity),detail.unit||item.unit)}`,left,totalY+54,width,28,700));}out.push(text(`Pagina ${page+1} di ${pageCount}`,left,3400,width,20,400,'#516e65'),'</svg>');urls.push(URL.createObjectURL(new Blob([out.join('')],{type:'image/svg+xml;charset=utf-8'})))}return urls}
  ...
  async function directNativeCommitmentItem(code){const requestId=globalThis.crypto?.randomUUID?.()||`commitment-${Date.now()}`,controller=new AbortController(),timer=setTimeout(()=>controller.abort(),30000);try{const url=globalThis.__technicsBridgeUrl+'/api/items/lookup?code='+encodeURIComponent(code)+'&commitmentDetails=native&commitmentRecovery='+Date.now(),response=await globalThis.fetch(url,{cache:'no-store',signal:controller.signal,headers:{'Cache-Control':'no-store','ngrok-skip-browser-warning':'1','X-Technics-Request-Id':requestId}}),payload=await response.json(),meta=payload?.meta||{},headers=response.headers,pairs=[['X-Technics-Request-Id','requestId'],['X-Technics-Node','nodeId'],['X-Technics-Node-Role','nodeRole'],['X-Technics-Version','version'],['X-Technics-Lease-Epoch','leaseEpoch'],['X-Technics-Server-Time','serverTime']],resolved=payload?.item;if(!response.ok||payload?.ok!==true||meta.dataAuthority!=='Technics'||meta.readOnly!==true||String(meta.requestId)!==requestId||pairs.some(([header,key])=>headers.get(header)!==String(meta[key]||''))||!qualifiedNativeSnapshot(resolved,code))throw Error(payload?.error||'Dettaglio Lotto non verificato.');return resolved}finally{clearTimeout(timer)}}
  async function loadNativeCommitmentItem(item){const code=String(item?.code||'').trim();if(!code)return item;try{const {response,payload}=await TechnicsDataClient.fetchJson(globalThis.__technicsBridgeUrl+'/api/items/lookup?code='+encodeURIComponent(code)+'&commitmentDetails=native&fresh='+Date.now(),{cache:'no-store',headers:{'Cache-Control':'no-store'}},{attempts:1,cacheMs:0,message:'Dettaglio Lotto temporaneamente non disponibile.'});const resolved=payload?.item;if(!response.ok||payload?.ok!==true||!qualifiedNativeSnapshot(resolved,code))throw Error(payload?.error||'Dettaglio Lotto non disponibile.');return resolved}catch{return directNativeCommitmentItem(code)}}
  let commitmentRequestSequence=0,commitmentSummaryRequestSequence=0;
function validateUnassignedDetail(item,now=Date.now()){
 const d=item?.commitmentDetail,u=d?.unassigned,r=d?.reconciliation,unit=String(d?.unit||item?.unit||'').trim().toUpperCase();
 if(!u||u.status!=='verified'||u.schemaVersion!==1||u.readOnly!==true||u.statusFilter!=='prodt_ordini.Stato=A'||u.source!=='prodr_ordini.QtaReale'||!Array.isArray(u.rows)||u.rows.length>500||u.rowCount!==u.rows.length||!Number.isFinite(Date.parse(u.readAt))||now-Date.parse(u.readAt)>120000||Date.parse(u.readAt)-now>30000)return null;
 const ids=new Set();let total=0;
 for(const x of u.rows){
  if(!Number.isSafeInteger(x.rowId)||x.rowId<=0||ids.has(x.rowId)||!Number.isSafeInteger(x.productionOrderId)||x.productionOrderId<=0||!Number.isInteger(x.productionOrderYear)||x.productionOrderYear<1900||x.productionOrderYear>9999||!Number.isFinite(x.quantity)||x.quantity<=0||String(x.unit||'').trim().toUpperCase()!==unit||x.lot&&x.location||x.op!==null&&typeof x.op!=='string'||x.ov!==null&&typeof x.ov!=='string')return null;
  const missing=[...(!x.lot?['lot']:[]),...(!x.location?['location']:[])];if(JSON.stringify(x.missingFields)!==JSON.stringify(missing))return null;
  ids.add(x.rowId);total+=x.quantity;
 }
 if(item?.availability?.committedQuantity==null||d.totalQuantity==null)return null;
 const inventory=Number(item?.availability?.committedQuantity),qualified=Number(d.totalQuantity),residual=inventory-qualified-total;
 if(!Number.isFinite(inventory)||!Number.isFinite(qualified)||!Number.isFinite(u.totalQuantity)||Math.abs(total-u.totalQuantity)>0.000001||r?.status!=='verified'||[r.inventoryQuantity,r.qualifiedQuantity,r.unassignedQuantity,r.unexplainedQuantity].some(v=>typeof v!=='number'||!Number.isFinite(v))||Math.abs(r.inventoryQuantity-inventory)>0.000001||Math.abs(r.qualifiedQuantity-qualified)>0.000001||Math.abs(r.unassignedQuantity-total)>0.000001||Math.abs(r.unexplainedQuantity-residual)>0.000001||r.matchesInventory!==(Math.abs(residual)<0.000001))return null;
 return {rows:u.rows,total,residual,inventory,qualified};
}
  function verifiedCommitment(resolved,expectedCode){
    const detail=resolved?.commitmentDetail||{},rows=commitmentRows(detail),total=Number(detail.totalQuantity),committed=Number(resolved?.availability?.committedQuantity),unit=String(detail.unit||resolved?.unit||'').trim().toUpperCase();
    const inventory=detail.inventoryCommittedQuantity==null?NaN:Number(detail.inventoryCommittedQuantity);
    if(String(resolved?.code||'').trim().toUpperCase()!==String(expectedCode||'').trim().toUpperCase()||(!rows.length&&!validateUnassignedDetail(resolved))||!Number.isFinite(total)||!Number.isFinite(committed)||!Number.isFinite(inventory)||Math.abs(inventory-committed)>0.01||rows.some(row=>!String(row.lot||'').trim()||!String(row.location||'').trim()||!Number.isFinite(Number(row.quantity))||Number(row.quantity)<=0||!String(row.op||'').trim()||(unit&&row.unit&&String(row.unit).trim().toUpperCase()!==unit)))return null;
    const rowKeys=rows.map(row=>JSON.stringify([row.productionOrderId,row.productionOrderYear,row.op,row.ov,row.lot,row.location,row.quantity,String(row.unit||unit).trim().toUpperCase()].map(value=>String(value??'').trim())));
    if(new Set(rowKeys).size!==rowKeys.length)return null;
    const sum=rows.reduce((value,row)=>value+Number(row.quantity),0);
    return Math.abs(sum-total)<=0.01?{detail,rows,total}:null;
  }
  function qualifiedNativeSnapshot(resolved,expectedCode){
    if(String(resolved?.code||'').trim().toUpperCase()!==String(expectedCode||'').trim().toUpperCase())return null;
    const availability=resolved?.availability||{},detail=resolved?.commitmentDetail||{},rows=commitmentRows(detail),committed=Number(availability.committedQuantity),total=Number(detail.totalQuantity);
    if(availability.dataAuthority!=='Technics'||availability.source!=='magaubicazioni_articolo'||!Number.isFinite(committed)||committed<0||!Number.isFinite(total)||total<0)return null;
    if(committed<=0.000001)return rows.length===0&&Math.abs(total)<=0.000001?{detail,rows,total}:null;
    return verifiedCommitment(resolved,expectedCode);
  }

  function renderUnassignedSection(overlay,item){
    let panel=overlay.querySelector('[data-unassigned-panel]');if(!panel){panel=document.createElement('section');panel.dataset.unassignedPanel='';overlay.querySelector('.inventorycommitmentscroll').append(panel)}
    const v=validateUnassignedDetail(item),u=item?.commitmentDetail?.unassigned;
    if(!v){panel.innerHTML='<h4>Impegni senza lotto/ubicazione</h4><p>Dati non disponibili o non aggiornati. Totale non verificato.</p>';return}
    const f=n=>esc(n.toLocaleString('it-IT',{minimumFractionDigits:2,maximumFractionDigits:4})+' '+String(item.unit||''));
    if(Math.abs(v.residual)<0.000001)overlay.querySelector('[data-inventory-commitments-total-label]').textContent='Totale dettagliato · Solo impegni qualificati';
    const check=overlay.querySelector('[data-inventory-commitments-check]');check.className=Math.abs(v.residual)<0.000001?'inventorycommitmentok':'inventorycommitmenterror';check.textContent=Math.abs(v.residual)<0.000001?'La differenza tra inventario e qualificati è spiegata dagli impegni senza assegnazione.':'Rimane una differenza residua tra inventario e impegni: vedere il confronto sotto.';
    panel.innerHTML='<h4>Impegni senza lotto/ubicazione</h4><p>Separati dagli impegni qualificati; nessun lotto o ubicazione viene assegnato automaticamente.</p>'+
      '<div class="unassignedsummary"><span>Inventario<br><b>'+f(v.inventory)+'</b></span><span>Qualificati<br><b>'+f(v.qualified)+'</b></span><span>Non assegnati<br><b>'+f(v.total)+'</b></span><span>Residuo<br><b>'+f(v.residual)+'</b></span></div>'+
      '<p>'+ (Math.abs(v.residual)<0.000001?'Totale riconciliato: qualificati + non assegnati = inventario.':'Differenza residua da verificare: '+f(v.residual))+'</p>'+
      (v.rows.length?'<table class="unassignedtable"><thead><tr><th>OP / anno</th><th>OV</th><th>Assegnazione</th><th>Quantità</th></tr></thead><tbody>'+v.rows.map(x=>'<tr><td>'+esc(x.op||'—')+'<br>'+esc(x.productionOrderYear)+'</td><td>'+esc(x.ov||'—')+'</td><td>Lotto: '+esc(x.lot||'non assegnato')+'<br>Ub.: '+esc(x.location||'non assegnata')+'</td><td>'+f(x.quantity)+'</td></tr>').join('')+'</tbody></table>':'<p>Nessun impegno senza assegnazione.</p>')+
      '<small>La stampa attuale contiene solo gli impegni qualificati. Questa sezione non è inclusa.</small>';
  }

  function openCommitments(item,summary){
    const overlay=commitmentDialog(),requestSequence=++commitmentRequestSequence,flag=overlay.querySelector('[data-inventory-overdue-only]'),tbody=overlay.querySelector('tbody'),preview=overlay.querySelector('[data-inventory-commitments-preview]'),check=overlay.querySelector('[data-inventory-commitments-check]');let currentItem=null;
    overlay.querySelector('h3').textContent=`${item.code||''} · ${item.name||item.description||''}`;
    const initialColumnCount=configureCommitmentColumns(overlay,needleCommittedItem(item));tbody.innerHTML=`<tr><td colspan="${initialColumnCount}">Caricamento dati aggiornati…</td></tr>`;
    flag.checked=false;flag.disabled=true;flag.onchange=null;overlay.querySelector('[data-unassigned-panel]')?.remove();
    overlay.querySelector('[data-inventory-commitments-overdue-total]').textContent='—';
    overlay.querySelector('[data-inventory-commitments-total]').textContent='—';
    overlay.querySelector('[data-inventory-commitments-total-label]').textContent='Totale impegnato';
    check.className='';check.textContent='Lettura aggiornata in corso.';
    if(preview){preview.disabled=true;preview.onclick=null}
    overlay.classList.remove('hidden');overlay.querySelector('[data-inventory-commitments-close]').focus({preventScroll:true});
    const paint=resolved=>{if(requestSequence!==commitmentRequestSequence)return;const verified=verifiedCommitment(resolved,item.code);if(!verified)throw Error('Dati aggiornati non coerenti.');currentItem=resolved;const showLotExpiry=needleCommittedItem(resolved),columnCount=configureCommitmentColumns(overlay,showLotExpiry),{detail,rows,total}=verified,overdueRows=rows.filter(row=>deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0);overlay.querySelector('h3').textContent=`${resolved.code||''} · ${resolved.name||resolved.description||''}`;const render=()=>{const visible=flag.checked?overdueRows:rows;tbody.innerHTML=visible.length?visible.map(row=>`<tr${deliveryOverdue(row.deliveryDate)&&Number(row.quantity)>0?' class="inventorycommitmentoverdue" data-delivery-overdue="true"':''}><td data-label="OP">${esc(row.op||'—')}</td><td data-label="OV">${esc(row.ov||'—')}</td><td class="inventorycommitmentlotlayout" data-label="Lotto / Layout"><span data-commitment-lot>${esc(row.lot||'—')}</span><small data-commitment-layout>Ub. ${esc(row.location||'—')}</small></td><td data-label="Data di consegna">${esc(date(row.deliveryDate))}</td>${showLotExpiry?`<td data-inventory-lot-expiry="true" data-label="Scadenza lotto">${esc(date(row.expiry))}</td>`:''}<td data-label="Impegnato">${esc(format(Number(row.quantity),row.unit||summary.unit))}</td></tr>`).join(''):`<tr><td colspan="${columnCount}">Nessun impegno nel filtro selezionato.</td></tr>`};flag.disabled=false;flag.onchange=render;render();const overdueTotal=overdueRows.reduce((sum,row)=>sum+Number(row.quantity),0);overlay.querySelector('[data-inventory-commitments-overdue-total]').textContent=format(overdueTotal,detail.unit||summary.unit);overlay.querySelector('[data-inventory-commitments-total]').textContent=format(total,detail.unit||summary.unit);const raw=Number(detail.rawInventoryCommittedQuantity),rawKnown=detail.rawInventoryCommittedQuantity!=null&&Number.isFinite(raw),matchesRaw=rawKnown&&Math.abs(raw-total)<=0.01;if(showLotExpiry){overlay.querySelector('[data-inventory-commitments-total-label]').textContent='Totale dettagliato · Solo impegni qualificati'}else{overlay.querySelector('[data-inventory-commitments-total-label]').innerHTML=matchesRaw?'Totale dettagliato <span class="inventorycommitmentverified">✓ Somma verificata</span>':(rawKnown?'Totale dettagliato <span class="inventorycommitmentunverified">⚠ Differenza inventario</span>':'Totale dettagliato <span class="inventorycommitmentunverified">⚠ Verifica inventario non disponibile</span>')};check.className=matchesRaw?'inventorycommitmentok':'inventorycommitmenterror';check.textContent=matchesRaw?'Dati aggiornati e somma verificata.':rawKnown?`Impegni qualificati ${format(total,detail.unit||summary.unit)}; inventario ${format(raw,detail.unit||summary.unit)}. Verificare la differenza.`:'Impegni qualificati; totale inventario non disponibile per la verifica.';if(!showLotExpiry)renderUnassignedSection(overlay,resolved);if(preview){preview.disabled=rows.length===0;preview.onclick=()=>openCommittedPrint(currentItem)}};
    loadNativeCommitmentItem(item).then(paint).catch(error=>{if(requestSequence!==commitmentRequestSequence)return;const columnCount=overlay.dataset.inventoryLotExpiry==='true'?6:5;tbody.innerHTML=`<tr><td colspan="${columnCount}">Dati aggiornati non disponibili.</td></tr>`;flag.disabled=true;check.className='inventorycommitmenterror';check.textContent=error?.message||'Dati aggiornati non disponibili.';if(preview){preview.disabled=true;preview.onclick=null}});
  }
  function installStyles() {
    ...
    style.textContent=`
  [data-unassigned-panel]{margin:8px 0;padding:8px;min-width:0;border:1px solid #aebfb7;text-align:center}
  [data-unassigned-panel] h4{margin:4px 0}
  [data-unassigned-panel] p,[data-unassigned-panel] small{font-size:11px;overflow-wrap:anywhere}
  .unassignedsummary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4px;font-size:11px}
  .unassignedtable{width:100%;table-layout:fixed;font-size:10px;border-collapse:collapse}
  .unassignedtable th,.unassignedtable td{text-align:center!important;vertical-align:middle!important;white-space:normal!important;overflow-wrap:anywhere;padding:4px 2px!important;border:1px solid #c7d7cf}
  .inventorycommitmentdialog table.unassignedtable thead th:nth-child(1){width:20%}
  .inventorycommitmentdialog table.unassignedtable thead th:nth-child(2){width:15%}
  .inventorycommitmentdialog table.unassignedtable thead th:nth-child(3){width:40%}
  .inventorycommitmentdialog table.unassignedtable thead th:nth-child(4){width:25%}
...
`;document.head.append(style);
  ...
  function apply(item, visibleRows) {
    ...
    const summarySequence=++commitmentSummaryRequestSequence,renderSummary=(resolved,value,commitmentReady)=>{if(summarySequence!==commitmentSummaryRequestSequence)return;host.innerHTML=markup(value,true,commitmentReady);host.removeAttribute('title');const commitmentButton=host.querySelector('[data-inventory-commitments="open"]');if(commitmentButton){const open=()=>openCommitments(resolved,value);commitmentButton.addEventListener('click',open);commitmentButton.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open()}})}};
    host.classList.add('inventoryavailability');host.classList.remove('zerototal');renderSummary(item,{total:null,free:null,committed:null,unit:summary.unit},false);
    loadNativeCommitmentItem(item).then(resolved=>{if(summarySequence!==commitmentSummaryRequestSequence)return;const nativeSummary=model(resolved.availability,resolved.availability?.totalQuantity,resolved.unit||item.unit||''),splitVerified=finite(nativeSummary.total)&&finite(nativeSummary.free)&&finite(nativeSummary.committed)&&Math.abs(nativeSummary.total-nativeSummary.free-nativeSummary.committed)<=0.01,commitmentReady=nativeSummary.committed>0&&!!verifiedCommitment(resolved,item.code);if(!splitVerified||nativeSummary.committed>0&&!commitmentReady)throw Error('Riepilogo impegnato non coerente.');renderSummary(resolved,nativeSummary,commitmentReady)}).catch(()=>{});
    ...
    if(finite(item.availability?.unassignedProductionCommittedQuantity)&&item.availability.unassignedProductionCommittedQuantity!==0){note.textContent+=`Impegni produzione non ripartiti per ubicazione: ${format(item.availability.unassignedProductionCommittedQuantity,summary.unit)}.`}note.hidden=!note.textContent.trim()
