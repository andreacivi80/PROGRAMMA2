(() => {
  ...
  const renderRelated = (item, suffix = "Apri") =>
    `<button type="button" class="formulaparent" data-formula-related-id="${Number(item.id) || 0}"><b>${esc(item.code)}</b><span>${esc(item.description)}</span><small>${esc(suffix)}</small></button>`;
