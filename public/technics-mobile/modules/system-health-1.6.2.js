(()=>{
  ...
  const state={ok:false,database:null,databaseLatencyMs:null,failures:0,lastCheck:"",lastSuccessAt:"",latencyMs:0,version:"",message:"Avvio controllo",errorCode:"CHECK-IN-CORSO"};
  let cycle=0;
  ...
  const ensurePanel=()=>{
    ...
    panel.querySelector("#retrySystemDiagnostics").addEventListener("click",()=>check());
  ...
  const start=()=>{
    ensurePanel();paint();const node=document.getElementById("net");node?.addEventListener("click",()=>document.getElementById("systemDiagnostics")?.classList.remove("hidden"));node?.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();node.click()}});
  ...
  window.TechnicsSystemHealth=Object.freeze({check,diagnostics:()=>Object.freeze({...state}),version:"1.7.20"});
  document.documentElement.dataset.systemHealth="1.7.20";
