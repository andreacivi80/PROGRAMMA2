(()=>{
  ...
  function validate(payload,responseHeaders,{minimumRevisions={},expectedIdentity=EXPECTED,now=Date.now(),expectedRequestId,requestFreshnessBounded=false}={}){
    ...
    const nativeContract=meta.source==='TechnicsNativeBridge'&&['1.9.352','1.9.353','1.9.354','1.9.357','1.9.359','1.9.360','1.9.361','1.9.362','1.9.363'].includes(meta.version);
    if(!matchedIdentity||!supportedVersion(meta.version)||(!nativeContract&&meta.source!=='TechnicsBridge')||meta.dataAuthority!=='Technics'||meta.readOnly!==true)throw fail('PACKING_MAIN_IDENTITY_INVALID','Identità del ponte principale non verificata.');
    ...
    if(fields.some(([h,k])=>typeof meta[k]!=='string'||!meta[k]||headers.get(h)!==meta[k])||!/^[A-Za-z0-9-]{1,128}$/.test(meta.requestId)||!(nativeContract?meta.leaseEpoch==='native-readonly':/^\d{1,20}$/.test(meta.leaseEpoch)))throw fail('PACKING_MAIN_IDENTITY_INVALID','Metadati e intestazioni del ponte non corrispondono.');
    ...
    else if(payload.stale===true||payload.offline===true||payload.degradedReadOnly===true||(!correlated&&(()=>{const httpAt=Date.parse(headers.get("Date")||"");return !Number.isFinite(httpAt)||Math.abs(httpAt-serverAt)>30000})()))throw fail('PACKING_MAIN_FRESHNESS_UNVERIFIED','Risposta precedente o data del ponte non verificabile; elenco non dichiarato aggiornato.');
    ...
    const nativeRoot=typeof payload.store?.root==='string'?payload.store.root.split(';')[0]:'';
    const nativeDiagnostics=payload.store?.diagnostics;
    const nativeLiveStore=nativeContract&&!emergency&&correlated&&requestFreshnessBounded===true
      &&payload.store?.mode==='native-shadow'&&payload.store.readOnlyList===true
      &&nativeRoot.startsWith(String.fromCharCode(92,92))&&nativeRoot.length>5
      &&((matchedIdentity.nodeRole==='primary'&&nativeDiagnostics===null&&!payload.store.root.includes(';'))
        ||(nativeDiagnostics?.degraded===false&&nativeDiagnostics.cacheFallback===null
          &&Array.isArray(nativeDiagnostics.failures)&&nativeDiagnostics.failures.length===0
          &&Array.isArray(nativeDiagnostics.availableRoots)&&nativeDiagnostics.availableRoots.includes(nativeRoot)));
    const authoritativeStore=!nativeContract&&payload.store?.available===true&&payload.store.authoritative===true&&payload.store.source==='server';
    const primaryMirror=!nativeContract&&payload.store?.available===true&&payload.store.authoritative===false&&payload.store.source==='mirror'&&!emergency&&correlated&&(!gatewayHeader||gatewayHeader==='stable-worker')&&matchedIdentity.nodeId==='technics-utente73-primary'&&matchedIdentity.nodeRole==='primary'&&Number.isSafeInteger(payload.store.fileCount)&&payload.store.fileCount>=rows.length&&payload.store.lastSyncAt==null;
    if(!authoritativeStore&&!primaryMirror&&!nativeLiveStore)throw fail('PACKING_MAIN_STORE_UNAVAILABLE','Archivio comune non confermato dal ponte.');
