(() => {
  ...
  style.textContent += '.pickmovementpanel th,.pickmovementpanel td{text-align:center}.pickmovementpanel .quantity{text-align:center}.pickmovementpanel th:last-child{white-space:nowrap}';
  style.textContent += '.pickmovementpanel th,.pickmovementpanel td{ text-align:center }.pickmovementpanel .quantity{text-align:center}.pickmovementpanel th:last-child{white-space:nowrap}.pickmovementcontrols{position:sticky;top:0;z-index:2;display:flex;align-items:center;justify-content:space-between;gap:6px;background:white;padding:6px 0;font-size:10px}.pickmovementcontrols button{flex:0 0 auto;padding:5px 7px;border:1px solid #a6c4b9;border-radius:6px;color:#155a48;background:#f3faf7;font:600 11px system-ui}.pickmovementstatus{color:#912f20;font-size:11px}';
  style.textContent += '.pickmovementpanel th,.pickmovementpanel td{vertical-align:middle}';
  style.textContent += '.pickmovementidentity{font-size:11px;line-height:1.35;overflow-wrap:anywhere;white-space:normal;max-width:100%}';
  style.textContent += '.pickmovementtotal{margin:4px 0;font-size:10px;text-align:center}.pickmovementstickyidentity{min-width:0;font-size:10px;font-weight:700;overflow-wrap:anywhere;text-align:center}';
  style.textContent += '.pickmovementinfo{font-style:italic!important}.pickmovementoverlay,.pickmovementpanel{overscroll-behavior:contain}.pickmovementoverlay{overflow:hidden}';
  document.head.append(style);

  const overlay = document.createElement('div');
  overlay.className = 'pickmovementoverlay';
  overlay.hidden = true;
  overlay.innerHTML = '<section class="pickmovementpanel" role="dialog" aria-modal="true" aria-labelledby="pickmovementtitle"><div class="pickmovementhead"><h2 id="pickmovementtitle">Elenco movimenti di magazzino</h2><button type="button" aria-label="Chiudi">×</button></div><p class="pickmovementidentity"></p><p class="pickmovementtotal"></p><div class="pickmovementscroll"></div></section>';
  document.body.append(overlay);
  let requestSequence = 0;
  let previousOverflow = null;
  const close = () => { requestSequence += 1; overlay.hidden = true; if(previousOverflow){ document.documentElement.style.overflow=previousOverflow.html; document.body.style.overflow=previousOverflow.body; previousOverflow=null; } };
  ...
  const compactOperator = movement => {
    ...
    const title = value => { const lower = String(value).toLocaleLowerCase('it-IT'); return lower ? Array.from(lower)[0].toLocaleUpperCase('it-IT') + Array.from(lower).slice(1).join('') : ''; };
    if (surname && given) return title(surname) + ' ' + Array.from(given.toLocaleUpperCase('it-IT'))[0] + '.';
    const full = String(movement.operator || surname).trim().replace(/\s+/g, ' ').toLocaleLowerCase('it-IT');
    const parts = full.split(' ');
    if (parts.length >= 2 && parts.every(part => /^\p{L}+$/u.test(part))) return title(parts.slice(0, -1).join(' ')) + ' ' + Array.from(parts.at(-1).toLocaleUpperCase('it-IT'))[0] + '.';
    return title(full) || '—';
  };
  ...
  const show = async (code, lot) => {
    ...
    if(overlay.hidden && !previousOverflow){ previousOverflow={html:document.documentElement.style.overflow,body:document.body.style.overflow}; document.documentElement.style.overflow='hidden'; document.body.style.overflow='hidden'; }
    ...
    try {
      ...
      url.searchParams.set('movementOffset','0');url.searchParams.set('movementLimit','50');
      ...
      const articleName = String(history.articleName || (String(payload.item?.code || '').trim() === code ? payload.item.name || payload.item.description : '') || '').trim() || 'Descrizione non disponibile';
      overlay.querySelector('.pickmovementidentity').textContent = code + ' · ' + articleName + ' · Lotto ' + lot;
      const firstPage=payload.item?.movementHistoryPage,meta=payload.meta||{};
      const movementPagesAvailable=meta.source==='TechnicsNativeBridge'&&((meta.nodeId==='technics-utente73-primary'&&meta.nodeRole==='primary')||(meta.nodeId==='technics-utente38-secondary'&&meta.nodeRole==='secondary'))&&firstPage?.readOnly===true&&firstPage?.dataAuthority==='Technics'&&firstPage.code===code&&firstPage.lot===lot&&firstPage.offset===0&&firstPage.limit===50&&firstPage.movementCount===history.movementCount&&Array.isArray(firstPage.movements)&&firstPage.movements.length<=50&&/^[a-f0-9]{64}$/.test(String(firstPage.snapshot||''));
      let expanded = false, allMovements = null;
      const render = () => {
      ...
      table.append(head);
      const rows = [history.initialLoading, ...(expanded ? allMovements : history.recentMovements)].filter(Boolean);
      for (const [index, movement] of rows.entries()) {
        ...
        const originalOperator=movement.operator||[movement.operatorSurname,movement.operatorGivenName].filter(Boolean).join(' ');tr.lastElementChild.title = originalOperator || '';
        tr.lastElementChild.setAttribute('aria-label', originalOperator || 'Operatore non disponibile');
      ...
      controls.className = 'pickmovementcontrols';
      const count = document.createElement('span');
      count.textContent = code + ' · Lotto ' + lot;
      count.className = 'pickmovementstickyidentity';
      overlay.querySelector('.pickmovementtotal').textContent = history.movementCount + ' movimenti totali · ' + (expanded ? 'tutti visibili' : history.recentMovements.length + ' più recenti');
      const toggle = document.createElement('button');
      toggle.type = 'button';toggle.textContent = expanded ? 'Ricompatta' : 'Espandi (' + history.movementCount + ')';
      toggle.setAttribute('aria-expanded',String(expanded));
      controls.append(count);if(movementPagesAvailable)controls.append(toggle);area.prepend(controls);
      toggle.addEventListener('click',async()=>{
        if(expanded){expanded=false;render();overlay.querySelector('.pickmovementpanel').scrollTop=0;return;}
        toggle.disabled=true;toggle.textContent='Caricamento…';
        try{
          if(!allMovements){
            let offset=0,snapshot=null;const collected=[],keys=new Set();
            do{
              const pageUrl=new URL(url);pageUrl.searchParams.set('movementOffset',String(offset));pageUrl.searchParams.set('movementLimit','50');if(snapshot)pageUrl.searchParams.set('movementSnapshot',snapshot);
              const pageController=new AbortController(),pageTimer=setTimeout(()=>pageController.abort(),12000);let pageResponse,body;try{pageResponse=await fetch(pageUrl,{cache:'no-store',signal:pageController.signal});body=await pageResponse.json();}finally{clearTimeout(pageTimer);}
              if(currentRequest!==requestSequence||overlay.hidden)return;
              const page=body.item?.movementHistoryPage;
              if(!pageResponse.ok||!body.ok||page?.readOnly!==true||page?.dataAuthority!=='Technics'||page.code!==code||page.lot!==lot||page.offset!==offset||page.limit!==50||page.movementCount!==history.movementCount||!Array.isArray(page.movements)||page.movements.length>50)throw new Error('Cronologia completa non disponibile o cambiata. Riaprire i movimenti.');
              if(!/^[a-f0-9]{64}$/.test(String(page.snapshot||''))||(snapshot!==null&&page.snapshot!==snapshot))throw new Error('Cronologia modificata durante la lettura. Riaprire i movimenti.');snapshot=page.snapshot;
              for(const movement of page.movements){
                if(!['magamovimenti','prodmovimenti'].includes(movement.source)||!Number.isSafeInteger(movement.id))throw new Error('Identità movimento non valida.');
                const key=movement.source+':'+movement.id;if(keys.has(key))throw new Error('Cronologia cambiata: movimento duplicato. Riaprire i movimenti.');keys.add(key);collected.push(movement);
              }
              const next=offset+page.movements.length;
              if(next>history.movementCount||page.complete!==(next===history.movementCount)||page.nextOffset!==(next===history.movementCount?null:next)||(!page.complete&&page.movements.length===0))throw new Error('Cronologia incompleta: nessun dato parziale mostrato.');
              offset=page.nextOffset;
            }while(offset!==null);
            if(collected.length!==history.movementCount)throw new Error('Conteggio cronologia non corrispondente.');
            allMovements=collected;
          }
          expanded=true;render();
        }catch(error){
          if(currentRequest!==requestSequence||overlay.hidden)return;
          toggle.disabled=false;toggle.textContent='Espandi ('+history.movementCount+')';
          let status=area.querySelector('.pickmovementstatus');if(!status){status=document.createElement('p');status.className='pickmovementstatus';controls.after(status);}status.textContent=error.message;
        }
      });
      if (!rows.length) { const empty = document.createElement('p'); empty.textContent = 'Nessun movimento trovato per questo lotto.'; area.append(empty); }
      };
      render();
    } catch (error) {
