...
try {
  ...
  const { storyBranchSupport, evaluateStorySpeech } = await server.ssrLoadModule("/src/storyBranches.ts");
  ...
  const naturalReplyAudit = Object.fromEntries(Object.entries(naturalReplies).map(([level,questions]) => [level,{questions:questions.length,invalid:questions.filter(question => question.options.length !== optionCountForLevel(level) || question.answer < 0 || question.answer >= question.options.length || new Set(question.options.map(option=>option.toLowerCase().trim())).size !== question.options.length || !question.situationIt.trim() || !question.prompt.trim() || !question.explanationIt.trim()).length}]));
  const storyBranchAudit = { episodes: storyEpisodes.length, supported: Object.keys(storyBranchSupport).length, invalid: storyEpisodes.filter(story => { const support = storyBranchSupport[story.id]; return !support || support.consequences.length !== story.choices.length || support.consequences.some(text => !text.trim()) || support.keywords.length < 3 || evaluateStorySpeech(story.model, support.keywords).percent < 67; }).map(story => story.id) };
  ...
  console.log(JSON.stringify({ total: mobileCurriculum.length, levels, minMinutes: Math.min(...mobileCurriculum.map(unit => unit.minutes)), maxMinutes: Math.max(...mobileCurriculum.map(unit => unit.minutes)), exerciseMinimums, reading, visualQuiz, visualLevelAudit, precisionAudit, naturalReplyAudit, storyBranchAudit, themePackAudit, wordGames, invalid, missing }, null, 2));
  ...
  if (missing.length || invalid.length || readingInvalid.length || visualMissing.length || readingPassages.length !== 10 || visualItems.length < 135 || visualLevelInvalid || a1B2VisualOverlap || Object.values(precisionAudit).some(row => row.questions !== 15 || row.invalid) || Object.values(naturalReplyAudit).some(row => row.questions !== 10 || row.invalid) || storyBranchAudit.episodes !== 15 || storyBranchAudit.supported !== 15 || storyBranchAudit.invalid.length || themePackInvalid.length || themePacks.length !== 42 || themePackAudit.social !== 5 || themePackAudit.dining !== 4 || themePackAudit.ira !== 4 || themePackAudit.accents !== 8 || themePackAudit.language !== 5 || themePackAudit.realLife !== 11 || themePackAudit.professional !== 5 || themePacks.filter(pack => pack.category === "accents").some(pack => !pack.authenticAudio?.url || !pack.sourceUrl) || Object.values(wordGames).some(game => game.crosswordWords < 8 || game.placedWords < 5 || game.hangmanPhrases < 25 || game.oppositePairs !== 15 || game.oppositeGroups !== 3 || game.invalidOpposites || game.repeatedOppositeTerms || game.invalidGroupSizes) || mobileCurriculum.length !== 60) process.exitCode = 1;
} finally {
  await server.close();
}
