...
try {
  ...
  const themePackInvalid = themePacks.filter(pack => !["2.7", "2.8"].includes(pack.introducedIn) || pack.guide.length < 4 || pack.vocabulary.length !== 8 || pack.questions.length !== 2 || pack.scenario.text.split(/\s+/).length < 20 || pack.questions.some(question => question.options.length < 3 || question.answer < 0 || question.answer >= question.options.length));
  const themePackAudit = { total: themePacks.length, social: themePacks.filter(pack => pack.category === "social").length, dining: themePacks.filter(pack => pack.category === "dining").length, ira: themePacks.filter(pack => pack.category === "ira").length, levels: Object.fromEntries(["A1", "A2", "B1", "B2", "C1"].map(level => [level, themePacks.filter(pack => pack.level === level).length])), invalid: themePackInvalid.map(pack => pack.id) };
  ...
  if (missing.length || invalid.length || readingInvalid.length || visualMissing.length || readingPassages.length !== 10 || visualItems.length < 81 || themePackInvalid.length || themePacks.length !== 13 || themePackAudit.social !== 5 || themePackAudit.dining !== 4 || themePackAudit.ira !== 4 || mobileCurriculum.length !== 60) process.exitCode = 1;
} finally {
  await server.close();
}