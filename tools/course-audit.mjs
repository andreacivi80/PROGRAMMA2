...
try {
  ...
  const themePackInvalid = themePacks.filter(pack => !["2.7", "2.8", "2.9", "3.0", "3.1", "7.3"].includes(pack.introducedIn) || pack.guide.length < 4 || pack.vocabulary.length !== 8 || pack.questions.length < 2 || pack.scenario.text.split(/\s+/).length < 20 || pack.questions.some(question => question.options.length < 3 || question.answer < 0 || question.answer >= question.options.length) || (pack.category === "real-life" && ["B1","B2","C1"].includes(pack.level) && pack.questions.some(question => question.options.length < (pack.level === "C1" ? 5 : 4))));
  ...
  if (missing.length || invalid.length || readingInvalid.length || visualMissing.length || readingPassages.length !== 10 || visualItems.length < 81 || themePackInvalid.length || themePacks.length !== 31 || themePackAudit.social !== 5 || themePackAudit.dining !== 4 || themePackAudit.ira !== 4 || themePackAudit.accents !== 8 || themePackAudit.language !== 5 || themePackAudit.realLife !== 5 || themePacks.filter(pack => pack.category === "accents").some(pack => !pack.authenticAudio?.url || !pack.sourceUrl) || Object.values(wordGames).some(game => game.crosswordWords < 8 || game.placedWords < 5 || game.hangmanPhrases < 5) || mobileCurriculum.length !== 60) process.exitCode = 1;
} finally {
  await server.close();
}
