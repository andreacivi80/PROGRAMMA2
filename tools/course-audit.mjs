...
try {
  ...
  console.log(JSON.stringify({ total: mobileCurriculum.length, levels, minMinutes: Math.min(...mobileCurriculum.map(unit => unit.minutes)), maxMinutes: Math.max(...mobileCurriculum.map(unit => unit.minutes)), exerciseMinimums, reading, visualQuiz, visualLevelAudit, precisionAudit, naturalReplyAudit, themePackAudit, wordGames, invalid, missing }, null, 2));
  ...
  if (missing.length || invalid.length || readingInvalid.length || visualMissing.length || readingPassages.length !== 10 || visualItems.length < 135 || visualLevelInvalid || a1B2VisualOverlap || Object.values(precisionAudit).some(row => row.questions !== 15 || row.invalid) || Object.values(naturalReplyAudit).some(row => row.questions !== 10 || row.invalid) || themePackInvalid.length || themePacks.length !== 42 || themePackAudit.social !== 5 || themePackAudit.dining !== 4 || themePackAudit.ira !== 4 || themePackAudit.accents !== 8 || themePackAudit.language !== 5 || themePackAudit.realLife !== 11 || themePackAudit.professional !== 5 || themePacks.filter(pack => pack.category === "accents").some(pack => !pack.authenticAudio?.url || !pack.sourceUrl) || Object.values(wordGames).some(game => game.crosswordWords < 8 || game.placedWords < 5 || game.hangmanPhrases < 25 || game.oppositePairs !== 15 || game.oppositeGroups !== 3 || game.invalidOpposites || game.repeatedOppositeTerms || game.invalidGroupSizes) || mobileCurriculum.length !== 60) process.exitCode = 1;
} finally {
  await server.close();
}
