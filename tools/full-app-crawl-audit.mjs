...
const inspect = async label => {
  ...
  return{label:${JSON.stringify(label)},title:document.querySelector("h1")?.textContent?.trim()||"",horizontal:document.documentElement.scrollWidth>innerWidth+1,duplicates:duplicates.slice(0,20),overflow:[...new Set(overflow)].slice(0,20),brokenWords:[...new Set(brokenWords)],overlaps:overlaps.slice(0,20),textBlocks:texts.length};
})()`);
    if (result) return result;
    await wait(300);
  }
  return { label, title: "", horizontal: true, duplicates: [], overflow: [], brokenWords: [], overlaps: [], textBlocks: 0 };
};
...
try {
  ...
  for (const unit of mobileCurriculum) {
    for (const phase of phases) {
      ...
      results.push(await inspect(`${unit.id}:${phase}`));
    }
  }
  ...
  const failures = results.flatMap(result => [result.horizontal ? `${result.label}: horizontal overflow` : null, result.duplicates.length ? `${result.label}: duplicate ${result.duplicates.join(" | ")}` : null, result.overflow.length ? `${result.label}: text overflow ${result.overflow.join(" | ")}` : null, result.brokenWords.length ? `${result.label}: broken words ${result.brokenWords.join(", ")}` : null, result.overlaps.length ? `${result.label}: overlaps` : null].filter(Boolean));
  console.log(JSON.stringify({ renderedScreens: results.length, lessonScreens: mobileCurriculum.length * phases.length, themeScreens: themePacks.length, failures: failures.slice(0,250), failureCount: failures.length }, null, 2));
