...
const inspect = async label => evaluate(`(() => {
  ...
  const scope=document.querySelector('[aria-modal="true"]')||document;
  const nodes=[...scope.querySelectorAll("button,input,select,textarea,summary,a")].filter(visible);
...
try {
  ...
  for (const viewport of [{ name: "phone", width: 390, height: 844, mobile: true }, { name: "desktop", width: 1440, height: 900, mobile: false }]) {
    ...
    await evaluate(`document.querySelector(".chooserStart")?.click()`); await wait(250);
    results.push(await inspect(`${viewport.name}-session-preview`));
    const previewState = await evaluate(`(()=>{const dialog=document.querySelector(".sessionPreviewBackdrop"),sheet=document.querySelector(".sessionPreviewSheet"),rect=sheet?.getBoundingClientRect();return{visible:Boolean(dialog&&sheet),skills:document.querySelectorAll(".sessionPreviewSkills>span").length,insideViewport:Boolean(rect&&rect.top>=0&&rect.bottom<=innerHeight&&rect.left>=0&&rect.right<=innerWidth)}})()`);
    results.push({ label: `${viewport.name}-session-preview-state`, ...previewState });
    if (viewport.name === "phone") await screenshot("phone-session-preview");
    ...
    await clickText("Focus"); await wait(200);
    const focusState = await evaluate(`({active:document.querySelector("main.app")?.classList.contains("focus-mode"),headerDisplay:getComputedStyle(document.querySelector("main.app>header")).display,exitVisible:Boolean([...document.querySelectorAll("button")].find(button=>button.textContent.includes("Esci")))})`);
    results.push({ label: `${viewport.name}-focus-mode`, ...focusState });
    results.push(await inspect(`${viewport.name}-focus-layout`));
    if (viewport.name === "phone") await screenshot("phone-focus-mode");
    await clickText("Esci"); await wait(150);
