...
try{
 ...
 const {supplementaryBankFor,supplementaryFingerprint}=await server.ssrLoadModule("/src/supplementaryQuiz.ts");
 const rows=mobileCurriculum.map(unit=>{
  ...
  return{id:unit.id,level:unit.cefr,bank:bank.length,exactOverlap,duplicatePrompts,duplicateFingerprints,highSimilarity};
 });
 ...
 const failures=rows.filter(row=>row.bank<15||row.exactOverlap||row.duplicatePrompts||row.duplicateFingerprints||row.highSimilarity);
